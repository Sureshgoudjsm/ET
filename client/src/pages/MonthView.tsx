...
import { CalendarDays, CheckCircle2, Clock, ChevronLeft, ChevronRight, Banknote } from "lucide-react";
...
export default function MonthView() {
  ...
  const { data: payments, isLoading } = trpc.payment.getByMonth.useQuery(
  ...
  const handleToggleStatus = (payment: NonNullable<typeof payments>[number]) => {
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">All transactions for a specific month</p>
      </div>
          ...
          <CardContent className="pt-5 pb-5">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                Pending in {selectedMonth?.label}
              </p>
          ...
          {payments && payments.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground text-sm">
                No transactions in {selectedMonth?.label} {year}
              </p>
            </div>
          )}
        </CardContent>
